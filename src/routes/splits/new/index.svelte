...
<BasePage>
    <div class="grid w-5/6 md:w-4/6 lg:w-3/6 xl:w-2/6 gap-3 px-5 h-full new-split place-items-center">
        ...
        <div class="grid grid-cols-2 rounded-md">
            ...
            <input type="text" class="text-center outline-none rounded-sm font-semibold shadow-black shadow-sm mr-5">
        </div>
        <div class="grid gap-2">
            {#each days as day, i}
                <div class="grid grid-cols-2 shadow-black bg-pink-700 rounded-full p-2 w-full">
                    ...
                    <input bind:value={workouts[i]} on:change="{update_value}" type="text" placeholder="Rest" class="text-center mr-5 outline-none p-0.5 rounded-sm font-medium shadow-black shadow-sm">
                </div>
            {/each}
        </div>
        <a class="text-white font-semibold text-lg py-1 border-pink-600 border-2 rounded-full px-4" href="workouts">
